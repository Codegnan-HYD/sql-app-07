-- > finding employees with parking spots
SELECT 
    e.employee_id,
    e.first_name,
    e.last_name,
    ps.parking_spot_id,
    ps.spot_number,
    ps.basement_level
FROM
    employees e
        INNER JOIN
    parking_spots ps ON e.parking_spot_id = ps.parking_spot_id;

-- > get all employees including those without parking spots
SELECT 
    e.employee_id,
    e.first_name,
    e.last_name,
    ps.parking_spot_id,
    ps.spot_number,
    ps.basement_level
FROM
    employees e
        LEFT JOIN
    parking_spots ps ON e.parking_spot_id = ps.parking_spot_id;

-- > get all parking spots including those unassigned
SELECT 
    ps.parking_spot_id,
    ps.spot_number,
    ps.basement_level,
    e.employee_id,
    e.first_name,
    e.last_name
FROM
    parking_spots ps
        LEFT JOIN
    employees e ON ps.parking_spot_id = e.parking_spot_id;

-- > get all employees and all parking spots
SELECT 
    e.employee_id,
    e.first_name,
    e.last_name,
    ps.parking_spot_id,
    ps.spot_number,
    ps.basement_level
FROM
    employees e
        LEFT JOIN
    parking_spots ps ON e.parking_spot_id = ps.parking_spot_id 
UNION SELECT 
    e.employee_id,
    e.first_name,
    e.last_name,
    ps.parking_spot_id,
    ps.spot_number,
    ps.basement_level
FROM
    employees e
        RIGHT JOIN
    parking_spots ps ON e.parking_spot_id = ps.parking_spot_id;

-- > find employees without parking spots
SELECT 
    e.employee_id, e.first_name, e.last_name
FROM
    employees e
        LEFT JOIN
    parking_spots ps ON e.parking_spot_id = ps.parking_spot_id
WHERE
    ps.parking_spot_id IS NULL;

-- > find unassigned parking spots
SELECT 
    ps.parking_spot_id, ps.spot_number, ps.basement_level
FROM
    parking_spots ps
        LEFT JOIN
    employees e ON ps.parking_spot_id = e.parking_spot_id
WHERE
    e.employee_id IS NULL;

-- > find employees with parking spots at a particular basement level
SELECT 
    e.employee_id,
    e.first_name,
    e.last_name,
    ps.spot_number,
    ps.basement_level
FROM
    employees e
        INNER JOIN
    parking_spots ps ON e.parking_spot_id = ps.parking_spot_id
WHERE
    ps.basement_level = 2;
    
    
-- > find unassigned parking spots at particular basement level
SELECT 
    ps.parking_spot_id, ps.spot_number, ps.basement_level
FROM
    parking_spots ps
        LEFT JOIN
    employees e ON ps.parking_spot_id = e.parking_spot_id
WHERE
    e.employee_id IS NULL
        AND ps.basement_level = 3;